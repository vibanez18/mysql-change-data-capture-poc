...
mysql --host mysqlmaster -uroot -p$MYSQL_MASTER_PASSWORD -AN -e "GRANT REPLICATION CLIENT ON *.* TO '$MYSQL_REPLICATION_USER'@'%' IDENTIFIED BY '$MYSQL_REPLICATION_PASSWORD';"
mysql --host mysqlslave -uroot -p$MYSQL_SLAVE_PASSWORD -AN -e "GRANT ALL ON maxwell.* TO '$MYSQL_MAXWELL_USER'@'%' IDENTIFIED BY '$MYSQL_MAXWELL_PASSWORD';"
mysql --host mysqlslave -uroot -p$MYSQL_SLAVE_PASSWORD -AN -e "GRANT SELECT, REPLICATION CLIENT, REPLICATION SLAVE ON *.* TO '$MYSQL_MAXWELL_USER'@'%';"
